...
export const renderYearsList = (
  ...
  yearsToRender.forEach(({ year, weeksCount }: YearWeeks) => {
    const yearElement: HTMLElement = createElementWithClass(
      "h2",
      "text-slate-50 text-xl font-bold text-center w-12",
      year.toString(),
    );
...
const renderDotsWeek = (yearWeeks: YearWeeks, dob: Date | null, currentDate: Date): HTMLElement => {
  const divElement: HTMLElement = createElementWithClass(
    "div",
